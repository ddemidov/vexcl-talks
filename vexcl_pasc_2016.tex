\documentclass[@BEAMER_OPTIONS@]{beamer}
    ...
    \lstset{
        ...
        morekeywords={global,kernel,ulong,size_t,get_global_id,get_global_size,omp,parallel}
        }
...
%----------------------------------------------------------------------------
\begin{frame}{Compiling compute kernels}
    \begin{itemize}
        \item JIT compilation:
            \begin{description}[OpenCL:\;\;]
                \item[OpenCL:] builtin functionality
                \item[CUDA:] generated sources are saved to disk and passed to\\
                    \code{nvcc} to create PTX module.
            \end{description}
            \vspace{\baselineskip}
        \item Offline kernel caching:
            \begin{description}[OpenCL:\;\;]
                \item[OpenCL:] save/load binaries
                \item[CUDA:] free side effect of the compilation process
            \end{description}
            \begin{itemize}
                \item SHA1 hash of the source is used as unique id of the
                    cached kernel.
            \end{itemize}
    \end{itemize}
\end{frame}
...
v = mul2(x + y);
            ...
            \begin{uncoverenv}<4>
                ...
                \end{itemize}
                \begin{tikzpicture}[remember picture,overlay]
                    \node[anchor=south east,yshift=0.25cm] at (current page.south east) {
                        \includegraphics[width=2.6cm]{quote}
                    };
                \end{tikzpicture}
            \end{uncoverenv}
        \end{onlyenv}
...
\begin{frame}
    \begin{figure}
        $\vcenter{\hbox{\includegraphics[width=3.6cm]{owl3}}}$
        \Large{Questions?}
    \end{figure}
\end{frame}
