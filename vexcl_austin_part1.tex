...
\begin{frame}[fragile]{Hello VexCL: vector sum}
    \begin{itemize}
        \item Can we use VexCL to achive same effect with a code that fits on
            one slide?
    \end{itemize}
    \pause
...
\begin{frame}[fragile]{Performance (Tesla K20c)}
    \begin{figure}
        \only<1|handout:0> {\includegraphics[width=0.9\textwidth]{perfcmp-1}}%
        \only<2|handout:0> {\includegraphics[width=0.9\textwidth]{perfcmp-2}}%
        \only<3|handout:0> {\includegraphics[width=0.9\textwidth]{perfcmp-3}}%
...
\begin{frame}{Summary}
    ...
    \begin{itemize}
        ...
        \item Supported compilers (minimal versions; don't forget to enable \Cpp{11} features):
            ...
            \end{itemize}
            \vspace{\baselineskip}
        \item[{[1]}] Denis Demidov, Karsten Ahnert, Karl Rupp, and Peter Gottschling.\\
