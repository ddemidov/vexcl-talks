...
\begin{frame}[fragile]{Permutations \singledevice}
    \begin{itemize}
        \item Two flavors:
            \begin{itemize}
                \item Index-based permutation
                \item Expression-based permutation
            \end{itemize}
    \end{itemize}
    \pause
...
y = reverse(x);
    ...
    \end{exampleblock}
    \pause
    \begin{exampleblock}{Both kinds are writable}
        \begin{lstlisting}
reverse(y) = x;
        \end{lstlisting}
    \end{exampleblock}
...
%----------------------------------------------------------------------------
\begin{frame}[fragile]{Fast Fourier Transform \additive \singledevice}
    \begin{itemize}
...
kernel void vexcl_multivector_kernel(ulong n,
    global double * res1, global double * res2,
    global double * prm1, double prm2,
    global double * prm3, double prm4
)
{
    for(size_t idx = get_global_id(0); idx < n; idx += get_global_size(0)) {
            double buf1 = ( ( prm1[idx] * prm2 ) - ( prm3[idx] * prm4 ) );
            double buf2 = ( ( prm1[idx] * prm4 ) + ( prm3[idx] * prm2 ) );
