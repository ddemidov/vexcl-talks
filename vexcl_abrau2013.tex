...
\begin{frame}{VexCL: библиотека шаблонов векторных выражений для OpenCL}
    ...
    \begin{itemize}
        \item \href{https://github.com/ddemidov/vexcl}{https://github.com/ddemidov/vexcl}
            \vspace{\baselineskip}
        \item Создана для облегчения разработки \Cpp приложений с применением
            технологии OpenCL.
...
%----------------------------------------------------------------------------
\begin{frame}{Использование VexCL c обобщенными алгоритмами \Cpp}
    \begin{description}[\quad]
        \item[AMGCL] --- алгебраический многосеточный метод решения СЛАУ
            ...
            \end{itemize}
            \vspace{\baselineskip}
        \item[Antioch] --- высокопроизводительные химические расчеты
            \begin{itemize}
...
%----------------------------------------------------------------------------
\begin{frame}[fragile]{Производительность простейшей реализации\footnote{
    ...
    \vspace{-1\baselineskip}
    \begin{itemize}
        \item Решение на GPU в 10 раз быстрее!
            \uncover<2->{\alert{Но,}}
            \begin{itemize}
                \item<2|alert@2> Метод Рунге-Кутты использует 4 временных
                    переменных состояния.
                \item<2|alert@2> Одна итерация метода приводит к запуску
                    нескольких ядер OpenCL.
            \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Генерация OpenCL-кода из алгоритмов Boost.odeint}
    \begin{itemize}
        \item VexCL включает тип \code{vex::generator::symbolic<T>}.
...
%----------------------------------------------------------------------------
\begin{frame}[fragile]{v2. Используем генератор функций и обобщенный функтор}
    \begin{exampleblock}{1. Объявим обобщенный функтор}
        \begin{lstlisting}
struct square_radius {
    ...
    T operator()(const T &x, const T &y) const {
        ...
        \end{lstlisting}
    \end{exampleblock}
    \begin{exampleblock}{2. Используем его для генерации VexCL-функции:}
        \begin{lstlisting}
...
R2 = sqr(X, Y);
    ...
    \begin{itemize}
        \item Функтор можно использовать как для CPU, так и для GPU-кода
    \end{itemize}
\end{frame}
...
%----------------------------------------------------------------------------
\begin{frame}[fragile]{v3. Используем генератор функций и библиотеку Boost.Phoenix}
    \begin{itemize}
        \item Ламбда-функции Boost.Phoenix являются ообщенными функторами
        \item Поэтому возможен следующий пример вариант:
    \end{itemize}
...
\begin{frame}{Заключение}
    ...
    \begin{itemize}
        \item VexCL позволяет писать прозрачный и компактный код без потери
            производительности.
        \item Генератор кода преобразует обобщенные алгоритмы \Cpp в OpenCL во
            время выполнения:
            \begin{itemize}
                \item Уменьшается число обращений к глобальной памяти.
                \item Снижается число запусков вычислительных ядер.
                \item Повторно используется код для CPU и GPU вариантов.
                    программы
            \end{itemize}
            \vspace{\baselineskip}
            \vspace{\baselineskip}
